 ...
 */
export const upsertSetting = async (key, value, label, updatedBy) => {
...
export const getCompanyRegisteredState = async () => {
    const now = Date.now();
    if (_cachedState && now < _cacheExpiry)
        return _cachedState;
 ...
 */
export const invalidateCompanyStateCache = () => {
    _cachedState = null;
    _cacheExpiry = 0;
};
