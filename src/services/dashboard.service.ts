...
export const getDashboardKPIs = async (role: string, cityId?: string, userId?: string) => {
  ...
  if (role === 'Technician' && userId) {
    const [assignedJobs, jobsToday, pendingJobs, completedJobs, upcomingJobs] = await Promise.all([
      ...
      prisma.job.count({ where: { assigned_user_id: userId, scheduled_start: { gte: todayStart, lte: todayEnd }, status: { not: 'Cancelled' } } }),
      ...
      prisma.job.count({ where: { assigned_user_id: userId, scheduled_start: { gte: now }, status: { notIn: ['Cancelled', 'Completed'] } } }),
    ]);
    return { assigned_jobs: assignedJobs, jobs_today: jobsToday, pending_jobs: pendingJobs, completed_jobs: completedJobs, upcoming_jobs: upcomingJobs };
  }
